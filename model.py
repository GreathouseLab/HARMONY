"""
GPT model + MuonAdamW optimizer extracted from train.py.

Lives in its own file so evaluate_probes.py (and any future inference scripts)
can import the model class without re-running training. train.py imports the
exact same classes from here.
"""
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, reduction='mean', return_hidden=False):
        """Standard forward returning loss/logits.

        return_hidden=True swaps the output for the post-final-norm hidden state
        (B, T, n_embd). Used by evaluate_probes.py to get read embeddings.
        """
        B, T = idx.size()
